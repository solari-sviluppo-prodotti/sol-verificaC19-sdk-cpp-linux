...
int main (int argc, char** argv) {
	...
	logger.info("---------- Test with Rules Updater ----------");
	{
		KeysStorageMemory keysStorage;
	...
	logger.info("---------- Test with dynamic library loading ----------");
	{
		...
		if (verificaC19Handle == NULL) {
			...
			pfDGCRulesKeysUpdater_isUpdated fDGCRulesKeysUpdater_isUpdated = (pfDGCRulesKeysUpdater_isUpdated)dlsym(verificaC19Handle, "DGCRulesKeysUpdater_isUpdated");
			...
			if (!fDGCRulesKeysUpdater_isUpdated(rulesKeysUpdater)) {
				logger.info("Rules or Keys are updating, waiting");
				while (!fDGCRulesKeysUpdater_isUpdated(rulesKeysUpdater)) {
					usleep(10000);
				}
			}
