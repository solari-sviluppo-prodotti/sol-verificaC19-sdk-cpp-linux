...
#include <verificaC19-sdk/examples/keys-storage/KeysStorageMemory.h>
#include <verificaC19-sdk/examples/keys-provider/KeysProviderTest.h>
#include <verificaC19-sdk/examples/keys-provider/KeysProviderItaly.h>
#include <verificaC19-sdk/examples/rules-storage/RulesStorageMemory.h>
#include <verificaC19-sdk/examples/rules-provider/RulesProviderTest.h>
#include <verificaC19-sdk/examples/rules-provider/RulesProviderItaly.h>
#include <verificaC19-sdk/examples/crl-storage/CRLStorageMemory.h>
#include <verificaC19-sdk/examples/crl-provider/CRLProviderTest.h>
...
int main (int argc, char** argv) {
	...
	LoggerStdout_c_info(logger, "---------- Test certificate, IT: Vaccine ----------");
	{
		...
		void* crlStorage = CRLStorageMemory_c_create();
		void* crlProvider = CRLProviderTest_c_create(logger);
		KeysProvider_c_refreshKeys(keysProvider, keysStorage);
		RulesProvider_c_refreshRules(rulesProvider, rulesStorage);
		CRLProvider_c_refreshCRL(crlProvider, crlStorage);
	...
	LoggerStdout_c_info(logger, "---------- Test certificate, IT: MOLECULAR test 2G mode ------");
	{
		...
		if (!DGCVerifier_c_verifyMinSdkVersion(verifier)) {
			...
			struct CertificateSimple_c* certificate = DGCVerifier_c_verify(verifier, "HC1:6BFOXN%TS3DH0YOJ58S S-W5HDC *M0II*%6C9B5G2+$NEJPP-IA%NGRIRJPC%OQHIZC4.OI:OIG/Q80P2W4VZ0K1H$$05QN*Y0K.G +AG5T01HJCAMKN$71Z95Z11VTO.L8YBJ-B93:GQBGZHHBIH5C99.B4DBF:F0.8ELG:.CC-8LQECKEBLDSH8XAG.6A-JE:GQA KX-SZDG0$JO+SW*PR+PHXF8IQV$K%OKOUFBBQR-S3D1PI0/7Q.H0807-L9CL62/2JJ11K2919GI1X1DDM8RMA0/41:6Z.2:NC-%CN$KJLCLF9+FJE 4Y3LL/II 05B9.Z8M+8:Y001HCY0R%0IGF5JNCPIGSUNG6YS75XJ/J0/V7.UI$RU8ZB.W2FI28LHUZUYZQNI9Y FQQGQ$FP DDVBDVBBX33UQLTU8L20H6/*12SADB9:G9J+9Y 5LJA8JF8JFHJP7NVDEBK3JQ7TI 05QNT+CCZ1ZA2I+T*R9XZ6/:COTJCURIF8CZPCJ4EF5LU5I-Q:.N$P9DX5NAM*PJYD3L2V0GBG.JL4LESU72S1CM%5OC%VSTJ8NC1TGO:QS02V505GJUTH", SCAN_MODE_2G);
			logCertificate(certificate, logger);
			CertificateSimple_c_release(certificate);
		}
		DGCVerifier_c_release(verifier);
	...
	LoggerStdout_c_info(logger, "---------- Test with Rules Updater without CRL ----------");
	{
		void* keysStorage = KeysStorageMemory_c_create();
		...
		void* updater = DGCUpdater_c_create(86400, rulesProvider, rulesStorage,
				keysProvider, keysStorage, NULL, NULL, logger);
		...
		void* verifier = DGCVerifier_c_create(keysStorage, rulesStorage, NULL, logger);
		if (!DGCVerifier_c_verifyMinSdkVersion(verifier)) {
			LoggerStdout_c_error(logger, "Minimum SDK version does not match");
		} else {
			struct CertificateSimple_c* certificate = DGCVerifier_c_verify(verifier, "HC1:6BFOXN%TS3DH0YOJ58S S-W5HDC *M0II*%6C9B5G2+$NEJPP-IA%NGRIRJPC%OQHIZC4.OI:OIG/Q80P2W4VZ0K1H$$05QN*Y0K.G +AG5T01HJCAMKN$71Z95Z11VTO.L8YBJ-B93:GQBGZHHBIH5C99.B4DBF:F0.8ELG:.CC-8LQECKEBLDSH8XAG.6A-JE:GQA KX-SZDG0$JO+SW*PR+PHXF8IQV$K%OKOUFBBQR-S3D1PI0/7Q.H0807-L9CL62/2JJ11K2919GI1X1DDM8RMA0/41:6Z.2:NC-%CN$KJLCLF9+FJE 4Y3LL/II 05B9.Z8M+8:Y001HCY0R%0IGF5JNCPIGSUNG6YS75XJ/J0/V7.UI$RU8ZB.W2FI28LHUZUYZQNI9Y FQQGQ$FP DDVBDVBBX33UQLTU8L20H6/*12SADB9:G9J+9Y 5LJA8JF8JFHJP7NVDEBK3JQ7TI 05QNT+CCZ1ZA2I+T*R9XZ6/:COTJCURIF8CZPCJ4EF5LU5I-Q:.N$P9DX5NAM*PJYD3L2V0GBG.JL4LESU72S1CM%5OC%VSTJ8NC1TGO:QS02V505GJUTH", SCAN_MODE_2G);
			...
			certificate = DGCVerifier_c_verify(verifier, "HC1:6BFOXN%TS3DH0YOJ58S S-W5HDC *M0II5XHC9B5G2+$N IOP-IA%NFQGRJPC%OQHIZC4.OI1RM8ZA.A5:S9MKN4NN3F85QNCY0O%0VZ001HOC9JU0D0HT0HB2PL/IB*09B9LW4T*8+DCMH0LDK2%K:XFE70*LP$V25$0Q:J:4MO1P0%0L0HD+9E/HY+4J6TH48S%4K.GJ2PT3QY:GQ3TE2I+-CPHN6D7LLK*2HG%89UV-0LZ 2ZJJ524-LH/CJTK96L6SR9MU9DHGZ%P WUQRENS431T1XCNCF+47AY0-IFO0500TGPN8F5G.41Q2E4T8ALW.INSV$ 07UV5SR+BNQHNML7 /KD3TU 4V*CAT3ZGLQMI/XI%ZJNSBBXK2:UG%UJMI:TU+MMPZ5$/PMX19UE:-PSR3/$NU44CBE6DQ3D7B0FBOFX0DV2DGMB$YPF62I$60/F$Z2I6IFX21XNI-LM%3/DF/U6Z9FEOJVRLVW6K$UG+BKK57:1+D10%4K83F+1VWD1NE", SCAN_MODE_3G);
			logCertificate(certificate, logger);
			CertificateSimple_c_release(certificate);
	...
	LoggerStdout_c_info(logger, "---------- Test with Rules Updater with CRL ----------");
	{
		void* keysStorage = KeysStorageMemory_c_create();
		void* keysProvider = KeysProviderItaly_c_create(logger);
		void* rulesStorage = RulesStorageMemory_c_create();
		void* rulesProvider = RulesProviderItaly_c_create(logger);
		void* crlStorage = CRLStorageMemory_c_create();
		void* crlProvider = CRLProviderItaly_c_create(logger);
	...
	LoggerStdout_c_info(logger, "---------- Test with dynamic library loading ----------");
	{
		...
		if (verificaC19Handle == NULL) {
			...
			pfDGCUpdater_c_create fDGCUpdater_c_create = (pfDGCUpdater_c_create)dlsym(verificaC19Handle, "DGCUpdater_c_create");
			pfDGCUpdater_c_release fDGCUpdater_c_release = (pfDGCUpdater_c_release)dlsym(verificaC19Handle, "DGCUpdater_c_release");
			pfDGCUpdater_c_isUpdated fDGCUpdater_c_isUpdated = (pfDGCUpdater_c_isUpdated)dlsym(verificaC19Handle, "DGCUpdater_c_isUpdated");

			void* updater = fDGCUpdater_c_create(86400, rulesProvider, rulesStorage, keysProvider, keysStorage, crlProvider, crlStorage, NULL);
			if (!fDGCUpdater_c_isUpdated(updater)) {
				LoggerStdout_c_info(logger, "Rules or Keys are updating, waiting");
				while (!fDGCUpdater_c_isUpdated(updater)) {
					usleep(10000);
				}
			}

			void* verifier = fDGCVerifier_c_create(keysStorage, rulesStorage, crlStorage, logger);
